...
# Mirror CLI, for `docker exec … bun run mirror:init|refresh|verify`. The shared
# context shim (_mirror-context.ts) is imported by the three named scripts, so it
# has to travel with them.
COPY --from=build /usr/src/app/scripts/csaf-mirror-init.ts \
                  /usr/src/app/scripts/csaf-mirror-refresh.ts \
                  /usr/src/app/scripts/csaf-mirror-verify.ts \
                  /usr/src/app/scripts/_mirror-context.ts \
                  ./scripts/

# Bun honors tsconfig `paths` at runtime — map `@/` to the compiled `./dist/` so
# the .ts scripts resolve their alias imports against the build output. In a dev
# checkout the source tsconfig.json maps @/* → ./src/*; in the image this emitted
# one maps @/* → ./dist/*. Same `bun run mirror:*` command in both environments.
...
RUN mkdir -p /var/log/cisa-cybersecurity-mcp-server && chown -R bun:bun /var/log/cisa-cybersecurity-mcp-server
